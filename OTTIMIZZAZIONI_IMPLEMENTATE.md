...
### ❌ 3. **Lazy Loading** - NON IMPLEMENTATO
**Stato:** Non implementato (architettura standalone components non richiede lazy loading esplicito per questo progetto MVP)

**Dove potrebbe essere implementato:**
- Route-level lazy loading con `loadChildren` in `app.routes.ts`
- Esempio futuro:
  ```typescript
  {
    path: 'campagne',
...
**Nota:** I standalone components già caricano solo ciò che serve, lazy loading sarebbe utile con moduli molto grandi.

---

### ⚠️ 4. **Query Precompilate** - PARZIALMENTE IMPLEMENTATO
**Cosa sono:** Query SQL con placeholder (`?`) per migliorare performance e sicurezza.

**Dove sono usate:**

#### ✅ Parametrized Queries (Sicurezza SQL Injection)
Tutti gli endpoint backend usano placeholder:

- **`server/routes/campagne.js`**
  - Linea 23: `SELECT * FROM campagne WHERE id = ?`
  - Linea 46: `INSERT INTO campagne (...) VALUES (?, ?, ?, ?)`
  - Linea 68: `SELECT * FROM campagne WHERE id = ?`
  - Linea 80: `UPDATE campagne SET giocatori = ? WHERE id = ?`
  - Linea 101: `UPDATE campagne SET giocatori = ? WHERE id = ?`

- **`server/routes/personaggi.js`**
  - Query parametrizzate con `?`

- **`server/routes/aggiornamenti.js`**
  - Query parametrizzate con `?`

#### ❌ Prepared Statements (Performance)
**Non implementato:** SQLite3 supporta prepared statements con `.prepare()` per performance, ma non sono usati nel progetto.

**Esempio di implementazione possibile:**
```javascript
const stmt = db.prepare('SELECT * FROM campagne WHERE gmId = ?');
const rows = stmt.all(gmId);
stmt.finalize();
```
